...
import com.revy.app.config.KafkaSseConsumer;
import com.revy.app.config.KafkaSseProducer;

@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
@EmbeddedKafka
class KotlinAppKafkaResourceIT {
    ...
    fun producesMessages() {
        ...
        restMockMvc.perform(post("/api/kotlin-app-kafka/publish?message=value-produce"))
    ...
    fun consumesMessages() {
        ...
        val mvcResult = restMockMvc
            .perform(get("/api/kotlin-app-kafka/register"))
            .andExpect(status().isOk)
        ...
        for (i in 0 until 100) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc
                    .perform(get("/api/kotlin-app-kafka/unregister"))
                return
            }
        }
